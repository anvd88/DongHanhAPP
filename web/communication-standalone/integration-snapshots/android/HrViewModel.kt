...
class HrViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // Business realtime dùng SSE durable; SignalR bên dưới chỉ còn communication chat/call/P2P.
    ...
    private fun onSignedIn(user: HrUser) {
        ...
        realtime.start(user.username) // communication legacy: chat/call/P2P
    ...
    fun runDiagnostics()=viewModelScope.launch{val ctx=getApplication<Application>();val map=linkedMapOf<String,String>();map["API"]=if(runCatching{repo.appConfig()}.isSuccess)"OK" else "Lỗi";map["SSE/Realtime"]=if(businessRealtime.isConnected())"Đã kết nối" else "Đang kết nối lại";map["SignalR Communication"]=if(realtime.isConnected())"Đã kết nối" else "Đang kết nối lại";map["FCM"]=if(pushToken.isNullOrBlank())"Chưa có token" else "OK";map["Camera"]=if(ContextCompat.checkSelfPermission(ctx,Manifest.permission.CAMERA)==PackageManager.PERMISSION_GRANTED)"Đã cấp quyền" else "Chưa cấp quyền";map["Micro"]=if(ContextCompat.checkSelfPermission(ctx,Manifest.permission.RECORD_AUDIO)==PackageManager.PERMISSION_GRANTED)"Đã cấp quyền" else "Chưa cấp quyền";map["TURN"]=if(repo.fetchTurnCreds()?.urls?.isNotEmpty()==true)"OK" else "Chưa cấu hình / không kết nối";diagnostics=map;supportTickets=runCatching{repo.mySupportTickets()}.getOrDefault(emptyList())}
    ...
    fun onAppResumed() {
        ...
        realtime.start(signedIn.user.username) // communication legacy: chat/call/P2P
    ...
    /**
     * Activity gọi khi app xuống nền: dừng poll + SSE + communication SignalR + nhịp tim để đỡ pin/mạng.
