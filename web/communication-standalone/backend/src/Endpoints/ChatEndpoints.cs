...
/// app_users (full_name), ảnh đại diện từ hr_employees.avatar, trạng thái online từ
...
public static class ChatEndpoints
{
    ...
    internal static async Task<Guid> GetOrCreateDirect(
        NpgsqlConnection conn, string me, string other, NpgsqlTransaction? transaction = null)
    {
        NpgsqlCommand Command(string sql) => transaction is null ? conn.Cmd(sql) : conn.Cmd(sql, transaction);
        var found = await Command(
            @"SELECT c.id
    ...
    private static async Task<List<ChatConversationDto>> ReadConversations(NpgsqlConnection conn, string me, Guid? onlyId, bool admin)
    {
        ...
        var cmd = conn.Cmd(
            $@"SELECT c.id, c.is_group, c.title, COALESCE(mine.is_pinned, FALSE) AS is_pinned,
               ...
               -- Ảnh đại diện: hr_employees.avatar là nguồn DUY NHẤT sau khi gộp (cùng ảnh với trang
               -- Hồ sơ web và màn Hồ sơ trên app). Ghép theo user_id trước, username sau — hồ sơ cũ
               -- có thể chưa gắn user_id.
               LEFT JOIN LATERAL (
                   SELECT oe.avatar AS image_data_url FROM hr_employees oe
                   WHERE oe.user_id = au.id OR lower(oe.username) = lower(o.username)
                   ORDER BY (oe.user_id = au.id) DESC NULLS LAST
                   LIMIT 1
               ) oav ON TRUE
               LEFT JOIN LATERAL (
                   SELECT BOOL_OR(us.is_active = TRUE AND us.last_seen >= CURRENT_TIMESTAMP - INTERVAL '90 seconds') AS is_online,
