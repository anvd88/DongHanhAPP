...
import { date, dateTime, initials, moneyVnd, num } from "../../lib/format";
import {
  docTypeLabel,
  ...
  leaveTypeLabel,
  type ManagerAlert,
  type ManagerAttendanceEmployee,
  type ManagerAttendanceStatus,
  type ManagerReport,
  type ManagerSummary,
  payoutMethodLabel,
  penaltySchedule,
...
import {
  getAppNotificationSettingStatus,
  setAppNotificationSettingEnabled,
  type AppNotificationSettingStatus,
} from "../../lib/appNotificationSettings";
...
export function HRHomePage() {
  ...
  return (
    <HrPage eyebrow="Tổng quan" title="Ketoanapk">
...
// Rút gọn User-Agent thành tên trình duyệt/hệ điều hành dễ đọc cho người dùng cuối.
function describeDevice(ua: string): string {
  ...
  const browser = /Edg\//i.test(ua) ? "Edge" : /Chrome\//i.test(ua) ? "Chrome" : /Firefox\//i.test(ua) ? "Firefox" : /Safari\//i.test(ua) ? "Safari" : "Trình duyệt";
  return [browser, os].filter(Boolean).join(" · ") || "Thiết bị";
}

// Quản lý thiết bị đăng nhập: liệt kê phiên web của chính tài khoản và thu hồi (đăng xuất từ xa).
function HRDeviceManager() {
  const { confirm, notify } = useAppNotifications();
  const { data, loading, reload } = useApi<LoginDevice[]>("/api/auth/devices");
  const [busy, setBusy] = useState<string | null>(null);

  const revoke = async (d: LoginDevice) => {
    const ok = await confirm({
      title: "Thu hồi thiết bị?",
      description: d.current ? "Đây là thiết bị hiện tại — bạn sẽ bị đăng xuất khỏi đây." : "Thiết bị này sẽ bị đăng xuất khỏi tài khoản của bạn.",
    ...
    setBusy(d.sid);
    try {
      await api.post(`/api/auth/devices/${encodeURIComponent(d.sid)}/revoke`, {});
      notify.success("Đã thu hồi thiết bị.");
      reload();
    } catch (err) {
      notify.error(err instanceof Error ? err.message : "Không thu hồi được thiết bị.");
    } finally {
      setBusy(null);
    }
  };
  ...
  return (
    <HrCard className="hr-devices-card">
      <div className="hr-card-head"><h2><MonitorSmartphone className="h-4 w-4" /> Thiết bị đăng nhập</h2></div>
      {loading && !data ? (
        <HrEmpty text="Đang tải danh sách thiết bị..." />
      ) : !data?.length ? (
        <HrEmpty text="Chưa ghi nhận thiết bị nào." />
      ) : (
        <div className="hr-devices-list">
          {data.map((d) => (
            <div key={d.sid} className="hr-device-row" data-revoked={d.revoked}>
              <div className="hr-device-info">
                <div className="hr-device-title">
                  {describeDevice(d.userAgent)}
                  {d.current && <span className="hr-device-badge">Hiện tại</span>}
                  {d.revoked && <span className="hr-device-badge is-revoked">Đã thu hồi</span>}
                </div>
                <div className="hr-device-meta">{d.lastSeen ? `Hoạt động ${dateTime(d.lastSeen)}` : "—"}</div>
              </div>
              {!d.revoked && (
                <HrButton tone="danger" onClick={() => revoke(d)} disabled={busy === d.sid}>
                  <Ban className="h-4 w-4" /> {busy === d.sid ? "Đang thu hồi..." : "Thu hồi"}
                </HrButton>
              )}
            </div>
          ))}
...
export function HRSystemSettingsPage() {
  ...
  const [notificationStatus, setNotificationStatus] = useState<AppNotificationSettingStatus | null>(null);
  ...
  useEffect(() => {
    ...
    const refreshNotificationStatus = async () => {
      try {
        const status = await getAppNotificationSettingStatus(user?.id);
        if (!cancelled) {
          setNotificationStatus(status);
          setSettingsError(null);
        }
      } catch {
        if (!cancelled) setSettingsError("Không đọc được quyền thông báo của ứng dụng.");
      }
    };
    const refreshWhenVisible = () => {
      if (!document.hidden) void refreshNotificationStatus();
    };
  ...
  const toggleAppNotifications = async () => {
    if (notificationToggling) return;

    const next = !(notificationStatus?.enabled ?? false);
    setNotificationToggling(true);
    setSettingsError(null);
    try {
      const status = await setAppNotificationSettingEnabled(user?.id, next);
      setNotificationStatus(status);
      if (next && !status.enabled) {
        notify.warning(
          status.supported
            ? "Hệ thống chưa cấp quyền thông báo cho KetoanAPK. Khi bật lại, ứng dụng sẽ hỏi quyền lần nữa."
            : "Thiết bị này chưa hỗ trợ thông báo ứng dụng.",
          "Thông báo vẫn đang tắt",
        );
      }
    } catch (e) {
      setSettingsError(e instanceof Error ? e.message : "Không đổi được cài đặt thông báo.");
    } finally {
  ...
  return (
    <HrPage eyebrow="Hệ thống" title="Cài đặt">
      ...
      {settingsError && <div className="hr-inline-error">{settingsError}</div>}

      <div className="hr-settings-list">
        <HRSettingToggle
          icon={<Bell />}
          title="Thông báo ứng dụng"
          meta={notificationMeta(notificationStatus, notificationToggling)}
          enabled={notificationStatus?.enabled ?? false}
      ...
      <HrCard className="hr-update-card">
        <div>
          ...
          <p>Phiên bản hiện tại: {currentVersionText}</p>
        </div>
...
function notificationMeta(status: AppNotificationSettingStatus | null, busy: boolean) {
  if (busy) return "Đang hỏi quyền hệ thống...";
  if (!status) return "Đang kiểm tra quyền";
  if (status.enabled) return "Đang bật";
  if (!status.supported) return "Thiết bị không hỗ trợ thông báo";
  if (!status.localEnabled && status.granted) return "Đang tắt";
  if (!status.systemEnabled) return "Hệ thống đang chặn thông báo";
  if (status.permission === "prompt" || status.permission === "prompt-with-rationale") return "Sẽ hỏi quyền khi bật";
...
function HRSettingToggle({
  ...
  return (
    <button type="button" className="hr-setting-toggle" data-on={enabled} onClick={onClick} disabled={disabled}>
...
type ManagerTab = "overview" | "attendance" | "approval" | "contracts" | "reports" | "alerts" | "catalog" | "employees" | "departments" | "shifts" | "assignments";

const managerTabs: Array<{ key: ManagerTab; label: string }> = [
  { key: "overview", label: "Tổng quan" },
  { key: "attendance", label: "Tình trạng" },
  { key: "approval", label: "Duyệt đơn" },
...
export function HRManagerPage() {
  ...
  return (
    <HrPage eyebrow="Quản trị" title="Quản lý nhân sự">
      <div className="hr-tabs">
        {managerTabs.map((item) => <button key={item.key} type="button" data-active={tab === item.key} onClick={() => setTab(item.key)}>{item.label}</button>)}
        {false && [
          ["employees", "Nhân viên"],
      ...
      {tab === "overview" && <HRManagerOverview onOpenTab={setTab} />}
      {tab === "attendance" && <HRManagerAttendance />}
      {tab === "approval" && <HRManagerApprovals />}
      {tab === "contracts" && <HRManagerContracts />}
      {tab === "reports" && <HRManagerReports />}
      {tab === "alerts" && <HRManagerAlerts />}
      {tab === "catalog" && <HRManagerCatalog />}
...
function HRManagerOverview({ onOpenTab }: { onOpenTab: (tab: ManagerTab) => void }) {
  const [day, setDay] = useState(todayKey());
  const [month, setMonth] = useState(currentMonth());
  const { data, loading, reload } = useApi<ManagerSummary>(`/api/hr/manager/summary?date=${day}&month=${month}`, [day, month]);
  const h = data?.headcount;

  return (
    <>
      <HrCard className="hr-manager-filter-card">
        <Field label="Ngày theo dõi"><Input type="date" value={day} onChange={(e) => setDay(e.target.value)} /></Field>
        <Field label="Kỳ báo cáo"><Input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></Field>
        <HrButton tone="secondary" onClick={() => reload()}><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Làm mới</HrButton>
      </HrCard>

      <div className="hr-manager-kpi-grid">
        <ManagerKpi icon={<Users />} label="Quân số" value={`${h?.active ?? 0}`} hint="Nhân viên đang làm" />
        <ManagerKpi icon={<UserCheck />} label="Có mặt" value={`${h?.present ?? 0}`} hint={`${h?.late ?? 0} đi muộn`} tone="success" />
        <ManagerKpi icon={<CalendarDays />} label="Nghỉ phép" value={`${h?.leave ?? 0}`} hint={`${h?.absent ?? 0} vắng`} tone="warning" />
        <ManagerKpi icon={<Gavel />} label="Công tác" value={`${h?.business ?? 0}`} hint={`${h?.unassigned ?? 0} chưa phân ca`} />
        <ManagerKpi icon={<ClipboardCheck />} label="Chờ duyệt" value={`${h?.pendingApprovals ?? 0}`} hint="Đơn cần xử lý" tone={h?.pendingApprovals ? "warning" : "neutral"} onClick={() => onOpenTab("approval")} />
        <ManagerKpi icon={<Hourglass />} label="Hợp đồng" value={`${h?.expiringContracts ?? 0}`} hint="Sắp hết hạn" tone={h?.expiringContracts ? "warning" : "neutral"} onClick={() => onOpenTab("contracts")} />
        <ManagerKpi icon={<Timer />} label="Tăng ca" value={`${h?.overtime ?? 0}`} hint="Nhân viên hôm nay" />
        <ManagerKpi icon={<AlertTriangle />} label="Cảnh báo" value={`${h?.alerts ?? 0}`} hint="Cần rà soát" tone={h?.alerts ? "danger" : "neutral"} onClick={() => onOpenTab("alerts")} />
      </div>
      ...
      <HrCard>
        <div className="hr-card-head">
          <div>
            ...
            <p>Có mặt, nghỉ phép, công tác và vắng theo từng ngày.</p>
          </div>
        </div>
        {data?.trend?.length ? <ManagerTrendChart points={data.trend} /> : <HrEmpty text={loading ? "Đang tải xu hướng..." : "Chưa có dữ liệu xu hướng."} />}
      </HrCard>
      ...
      <HrCard>
        <div className="hr-card-head">
          <div>
            ...
            <p>Quân số hiện diện trong ngày đã chọn.</p>
          </div>
          <button type="button" onClick={() => onOpenTab("attendance")} aria-label="Xem tình trạng"><ChevronRight className="h-4 w-4" /></button>
        </div>
        <div className="hr-manager-dept-list">
          {data?.departments?.length ? data.departments.map((dep) => (
            <article key={dep.departmentId || dep.departmentName} className="hr-manager-dept-row">
              <div>
                <strong>{dep.departmentName}</strong>
                <small>{dep.total} nhân viên · {dep.present} có mặt</small>
              </div>
              <div>
                <span data-tone="warning">{dep.leave + dep.business}</span>
                <span data-tone="danger">{dep.absent}</span>
              </div>
            </article>
          )) : <HrEmpty text={loading ? "Đang tải phòng ban..." : "Chưa có dữ liệu phòng ban."} />}
        </div>
      </HrCard>
    </>
  );
...
function ManagerKpi({
  ...
  const content = (
    <>
      <span className="hr-manager-kpi-icon">{icon}</span>
      <span className="hr-manager-kpi-copy">
        <small>{label}</small>
        <strong>{value}</strong>
        <em>{hint}</em>
      </span>
  ...
  return onClick ? (
    <button type="button" className="hr-manager-kpi" data-tone={tone} onClick={onClick}>{content}</button>
  ) : (
    <article className="hr-manager-kpi" data-tone={tone}>{content}</article>
  );
}

function ManagerTrendChart({ points }: { points: ManagerSummary["trend"] }) {
  const max = Math.max(1, ...points.map((p) => p.present + p.leave + p.business + p.absent));
  return (
    <div className="hr-manager-trend" aria-label="Xu hướng quân số">
      {points.map((p) => (
        <div key={p.date} className="hr-manager-trend-col">
          <div>
            <span data-tone="success" style={{ height: `${Math.max(8, (p.present / max) * 100)}%` }} title={`${p.present} có mặt`} />
            <span data-tone="warning" style={{ height: `${Math.max(4, ((p.leave + p.business) / max) * 100)}%` }} title={`${p.leave + p.business} nghỉ/công tác`} />
            <span data-tone="danger" style={{ height: `${Math.max(4, (p.absent / max) * 100)}%` }} title={`${p.absent} vắng`} />
          </div>
          <small>{p.label}</small>
        </div>
      ))}
...
const managerStatusOptions: Array<{ value: ManagerAttendanceStatus; label: string }> = [
  ...
  { value: "unassigned", label: "Chưa phân ca" },
];

function HRManagerAttendance() {
  const [day, setDay] = useState(todayKey());
  const [status, setStatus] = useState<ManagerAttendanceStatus>("all");
  const [departmentId, setDepartmentId] = useState("");
  const query = `/api/hr/manager/attendance?date=${day}&status=${status}${departmentId ? `&departmentId=${departmentId}` : ""}`;
  const { data, loading, reload } = useApi<ManagerAttendanceEmployee[]>(query, [day, status, departmentId]);
  const { data: departments } = useApi<Department[]>("/api/hr/departments");

  return (
    <>
      <HrCard className="hr-manager-filter-card">
        <Field label="Ngày"><Input type="date" value={day} onChange={(e) => setDay(e.target.value)} /></Field>
        <Field label="Trạng thái">
          <Select value={status} onChange={(e) => setStatus(e.target.value as ManagerAttendanceStatus)} className="w-full">
            {managerStatusOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}
          </Select>
        </Field>
      ...
      <HrCard>
        <div className="hr-card-head">
          <div>
            ...
            <p>{data?.length ?? 0} nhân viên theo bộ lọc hiện tại.</p>
          </div>
        </div>
        <div className="hr-manager-employee-list">
          {loading && !data ? <HrEmpty text="Đang tải tình trạng nhân viên..." /> : data?.length ? data.map((emp) => <ManagerEmployeeRow key={emp.employeeId} emp={emp} />) : <HrEmpty text="Không có nhân viên phù hợp bộ lọc." />}
        </div>
      </HrCard>
    </>
  );
...
function statusTone(status: ManagerAttendanceStatus): Tone {
  if (status === "present") return "success";
  if (status === "late" || status === "leave" || status === "business") return "warning";
  if (status === "absent") return "danger";
  return "muted";
}

function ManagerEmployeeRow({ emp }: { emp: ManagerAttendanceEmployee }) {
  return (
    <article className="hr-manager-employee-row">
      <span className="hr-person-avatar">{emp.avatar ? <img src={emp.avatar} alt="" /> : initials(emp.employeeName)}</span>
      <div>
        <strong>{emp.employeeName}</strong>
        <small>{emp.employeeCode} · {emp.position || "Nhân viên"} · {emp.departmentName || "Chưa có phòng ban"}</small>
        <em>{emp.shiftName || "Chưa phân ca"} · Vào {emp.checkIn || "--:--"} · Ra {emp.checkOut || "--:--"}</em>
        {(emp.requestNo || emp.requestTitle) && <em>{emp.requestNo || "Đơn"} · {emp.requestTitle || emp.statusLabel}</em>}
      </div>
      <HrStatus status={statusTone(emp.status)}>{emp.statusLabel}</HrStatus>
    </article>
  );
}

function HRManagerApprovals() {
  const { notify } = useAppNotifications();
  const { data, loading, reload } = useApi<RequestListItem[]>("/api/requests?scope=inbox");
  const [activeId, setActiveId] = useState<string | null>(null);
  ...
  return (
    <>
      <div className="hr-grid-3">
        <HrStat label="Chờ duyệt" value={`${pending}`} hint="Đơn cần xử lý" tone={pending ? "warning" : "success"} />
        <HrStat label="Tổng trong hộp" value={`${data?.length ?? 0}`} hint="Theo quyền duyệt" />
        <HrStat label="Thao tác" value="1 chạm" hint="Duyệt hoặc từ chối trên app" />
      </div>
      <HrCard>
        {loading && !data ? <HrEmpty text="Đang tải đơn chờ duyệt..." /> : data?.length ? data.map((item) => <RequestRow key={item.id} item={item} onClick={() => setActiveId(item.id)} />) : <HrEmpty text="Không có đơn nào chờ bạn duyệt." />}
      </HrCard>
...
function HRManagerContracts() {
  const [days, setDays] = useState("60");
  const { data, loading, reload } = useApi<ExpiringContract[]>(`/api/hr/manager/contracts/expiring?days=${days}`, [days]);
  return (
    <>
      <HrCard className="hr-manager-filter-card">
        <Field label="Khoảng cảnh báo">
          <Select value={days} onChange={(e) => setDays(e.target.value)} className="w-full">
      ...
      <HrCard>
        <div className="hr-card-head"><h2>Hợp đồng sắp hết hạn</h2></div>
        {loading && !data ? <HrEmpty text="Đang tải hợp đồng..." /> : data?.length ? data.map((item) => (
          <article key={item.id} className="hr-manager-contract-row">
            <div>
              <strong>{item.employeeName}</strong>
              <small>{item.employeeCode} · {item.departmentName || "Chưa có phòng ban"}</small>
              <em>{item.contractNo || item.contractType || "Hợp đồng"} · Hết hạn {item.endDate ? date(item.endDate) : "--"}</em>
            </div>
            <HrStatus status={item.daysLeft <= 7 ? "danger" : "warning"}>{item.statusLabel}</HrStatus>
          </article>
        )) : <HrEmpty text="Không có hợp đồng sắp hết hạn trong khoảng này." />}
      </HrCard>
    </>
  );
}

function HRManagerReports() {
  const [month, setMonth] = useState(currentMonth());
  const { data, loading, reload } = useApi<ManagerReport>(`/api/hr/manager/reports?month=${month}`, [month]);
  const t = data?.totals;
  const c = data?.headcountChange;
  return (
    <>
      <HrCard className="hr-manager-filter-card">
        <Field label="Kỳ báo cáo"><Input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></Field>
      ...
      <div className="hr-grid-4">
        <HrStat label="Công" value={num(t?.workedDays ?? 0, 1)} hint={`Vắng ${num(t?.absentDays ?? 0, 1)} ngày`} />
        <HrStat label="Phép" value={num(t?.leaveDays ?? 0, 1)} hint="Ngày nghỉ đã duyệt" tone="warning" />
        <HrStat label="Tăng ca" value={`${num(t?.overtimeHours ?? 0, 1)}h`} hint={`${num(t?.lateDays ?? 0, 0)} lượt đi muộn`} />
        <HrStat label="Biến động" value={`+${c?.newHires ?? 0} / -${c?.inactiveThisMonth ?? 0}`} hint={`${c?.activeEnd ?? 0} đang làm`} />
      </div>
      <HrCard>
        <div className="hr-card-head">
          <div>
            <h2>Theo phòng ban</h2>
            <p>Công, phép, tăng ca và vắng trong kỳ.</p>
          </div>
        </div>
        <div className="hr-manager-report-list">
          {loading && !data ? <HrEmpty text="Đang tải báo cáo..." /> : data?.byDepartment?.length ? data.byDepartment.map((row) => (
            <article key={row.departmentName} className="hr-manager-report-row">
              <strong>{row.departmentName}</strong>
              <div>
                <span>Công <b>{num(row.workedDays, 1)}</b></span>
                <span>Phép <b>{num(row.leaveDays, 1)}</b></span>
                <span>Tăng ca <b>{num(row.overtimeHours, 1)}h</b></span>
                <span>Vắng <b>{num(row.absentDays, 1)}</b></span>
              </div>
            </article>
          )) : <HrEmpty text="Chưa có dữ liệu báo cáo trong kỳ." />}
        </div>
      </HrCard>
      <HrCard>
        <div className="hr-card-head"><h2>Đơn từ trong kỳ</h2></div>
        <div className="hr-manager-report-list is-compact">
          <ListRow title="Chờ duyệt" meta="Đơn chưa hoàn tất luồng duyệt" status={`${t?.pendingRequests ?? 0}`} />
          <ListRow title="Đã duyệt" meta="Đơn đã áp dụng nghiệp vụ" status={`${t?.approvedRequests ?? 0}`} />
          <ListRow title="Từ chối" meta="Đơn không được chấp thuận" status={`${t?.rejectedRequests ?? 0}`} />
        </div>
      </HrCard>
    </>
  );
...
function HRManagerAlerts() {
  const [month, setMonth] = useState(currentMonth());
  const { data, loading, reload } = useApi<ManagerAlert[]>(`/api/hr/manager/alerts?month=${month}`, [month]);
  return (
    <>
      <HrCard className="hr-manager-filter-card">
        <Field label="Kỳ cảnh báo"><Input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></Field>
        <HrButton tone="secondary" onClick={() => reload()}><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /> Làm mới</HrButton>
      </HrCard>
      <HrCard>
        <div className="hr-card-head">
          <div>
            <h2>Cảnh báo nhân sự</h2>
            <p>Nghỉ nhiều, đi muộn thường xuyên và hợp đồng cần xử lý.</p>
          </div>
        </div>
        <div className="hr-manager-alert-list">
          {loading && !data ? <HrEmpty text="Đang tải cảnh báo..." /> : data?.length ? data.map((item) => (
            <article key={item.id} className="hr-manager-alert-row" data-level={item.level}>
              <span>{item.level === "danger" ? <FileWarning className="h-5 w-5" /> : <AlertTriangle className="h-5 w-5" />}</span>
              <div>
                <strong>{item.title}</strong>
                <small>{item.employeeName} · {item.employeeCode} · {item.departmentName || "Chưa có phòng ban"}</small>
                <em>{item.message}</em>
              </div>
              <b>{item.metricLabel}</b>
            </article>
          )) : <HrEmpty text="Không có cảnh báo trong kỳ này." />}
        </div>
      </HrCard>
    </>
  );
