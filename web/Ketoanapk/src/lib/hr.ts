...
export type ManagerAttendanceStatus = "all" | "present" | "late" | "leave" | "business" | "absent" | "unassigned";

export interface ManagerHeadcount {
  total: number;
  active: number;
  present: number;
  leave: number;
...
export interface ManagerDepartmentStatus {
  departmentId?: string | null;
  departmentName: string;
  total: number;
  present: number;
...
export interface ManagerSummary {
  ...
  departments: ManagerDepartmentStatus[];
  trend: ManagerTrendPoint[];
}

export interface ManagerAttendanceEmployee {
  employeeId: string;
  employeeCode: string;
  employeeName: string;
  position: string;
...
export interface ManagerDepartmentReport {
  departmentName: string;
  workedDays: number;
  absentDays: number;
  lateDays: number;
...
export interface ManagerReport {
  ...
  byDepartment: ManagerDepartmentReport[];
}
...
export const requestFields: Record<string, RequestField[]> = {
  ...
  business_trip: [
    { key: "fromDate", label: "Từ ngày", type: "date" },
    { key: "toDate", label: "Đến ngày", type: "date" },
    { key: "location", label: "Địa điểm công tác", type: "text" },
    { key: "reason", label: "Nội dung công tác", type: "textarea" },
  ],
  overtime: [
