    ...
    ILogger<ChangeWatcher> logger) : BackgroundService
{
    ...
    private const string TokenSql = @"
        SELECT CONCAT(
            ...
            (SELECT ISNULL(CONVERT(VARCHAR(20), CHECKSUM_AGG(CHECKSUM(id, loai_phieu, doi_tac, nhan_vien, ngay_lap, han_hoan_thanh))), '0') FROM dbo.gia_cong_phieu), '|',
            (SELECT COUNT(*) FROM dbo.gia_cong_hang_hoa), '|',
            (SELECT ISNULL(CONVERT(VARCHAR(20), CHECKSUM_AGG(CHECKSUM(phieu_id, loai_dong, ten_hang, quy_cach, don_vi_tinh, so_luong, don_gia_gia_cong))), '0') FROM dbo.gia_cong_hang_hoa), '|',
